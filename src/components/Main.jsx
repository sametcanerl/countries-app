...
const Main = () => {
  ...
  useEffect(() => {
    const res = countries?.filter((search) => {
      return seacrhTerm.any
        ? search.region?.toLowerCase().match(seacrhTerm.any.toLowerCase().trim()) ||
            search.name?.toLowerCase().match(seacrhTerm.any.toLowerCase().trim()) ||
            search.capital?.toLowerCase().match(seacrhTerm.any.toLowerCase().trim())
        : search.name?.toLowerCase().match(seacrhTerm.country.toLowerCase().trim());
    });
  ...
  return (
    <div className="container">
      ...
      <div className="input-group input-group w-50  mt-4 ">
